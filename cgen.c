// Generate potentially good starting sequence strings.
// (c) 2014-5 duane a. bailey

// These strings meet:
//  P2: start with 2
//  P3: contain no double 3's
//  P4: contain no substrings, V^4
// Additionally, the -x switch allows filtering on:
//  P0: ends with number that is *not* the curl.
...
void Usage(char *pn)
{
  ...
  fprintf(stderr,"\t-x\tOnly generate strings not ending in correct curl.\n");
  exit(1);
}
